...
public class PolyService {
    public void ex1(){
        ...
        // (1-1) car (부모 = 부모) Car 메소드 모두 사용 가능
        ...
        car2.setFuel("전기");
        car2.setWheel(4);
//        car2.setBattery(1000000);        // (1-2) tesla (자식 = 자식) tesla 메소드 모두 사용 가능
...
//        car3.setDiscountOffer(0.5); // 오류 발생
        ...
        // + 다형성 적용 -> 부모 타입 참조 자료형의 변수를 하나의 묶음으로 다루는것

        Car[] arr = new Car[3]; // 부모타입 참조변수 배열 선언 및 할당 (각 배열 요소가 Car타입 참조 변수)
        arr[0] = car; // Car 주소 == Car객체
...
//        Car[] arr = {new Car(), new Tesla(), new Spark()};
        ...
        for(int i=0; i<arr.length; i++){
            if(arr[i] instanceof  Tesla) System.out.println("Tesla 객체 입니다.");
            else if(arr[i] instanceof  Spark) System.out.println("Spark 객체 입니다.");
            else System.out.println("Car 객체 입니다.");
        }
    }

    // 전달받은 Car 또는 자식 객체의 엔진, 연료, 바퀴 개수를 출력하는 메소드
    // 매개변수에 부모 타입 참조변수를 작성하면 모든 자식 객체를 전달 받을 수 있다.
    public void printCar(Car c){ // 부모타입 탐조변수 = 자식 객체 주소
        // 매개변수에 작성된 참조형 변수에는 주소가 저장된다.(얕은 복사)
        // 메소드 내부 변수 + 매개변수 == 지역 변수 (Local Variable)
        System.out.println("엔진 : "+c.getEngine());
        System.out.println("연료 : "+c.getFuel());
        System.out.println("바퀴 개수 : "+c.getWheel());
    }

    // 전달 받은 매개변수에 따라서 Car 또는 자식 객체를 생성하고 생성된 객체의 주소를 반환
    ...
    public void ex4(){
        ...
        else System.out.println("실패(Spark 타입이 아님");
    }
    ...
    public void ex5(){
        ...
        //   c1.getEngine()호출 코드와
        //   edu.kh.poly.ex1.model.vo.Car.getEngine() 메소드 코드를 연결 --> [정적 바인딩]
        ...
        // edu.kh.poly.ex1.model.vo.Car.toString()
        // 참조변수 c2가 Car 타입이므로 toString()도 Car의 toString()을 호출 - 정적 바인딩

        // 하지만 실행 해보면 자식(Spark)의 toString()이 호출되는 것을 확인 가능
        // --> 컴파일 시에는 부모(Car)와 바인딩 == [정적 바인딩]
        // --> "실행 시"에는 자식(Spark)의 오버라이딩 된 메소드와 바인딩 == [동적 바인딩]
