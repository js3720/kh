...
// 2) 객체에 특정 key가 존재하지않으면 추가할 수 있다
...
// 유효성 검사 진행 여부 확인용 객체
// 모든 value가 전부 true일 경우에만 회원가입 진행
const checkObj = {
    "memberEmail" : false,
    "memberPw" : false,
...
memberEmail.addEventListener("input",()=>{
    ...
    if(regEx.test(memberEmail.value)){
        ...
        .then(resp => resp.text()) // 응답객체 -> 파싱parsing, 데이터 형태 변환)
        .then(count => {
...
memberPw.addEventListener("input", ()=>{
    ...
    if(regEx.test(memberPw.value)){
        ...
        if(memberPwConfirm.value.trim().length==0){
            ...
            if(memberPw.value == memberPwConfirm.value){ // 비밀번호 == 비밀번호 확인 같을경우
...
memberTel.addEventListener("input", ()=>{
    if(memberTel.value.trim().length==0){ // 전화번호가 입력이 되지 않은 경우
    ...
    for(let key in checkObj){
        if(!checkObj[key]){
            ...
            switch(key){
                ...
                case "memberPw" :
                    alert("비밀번호가 일치하지 않습니다."); break;
                case "memberPwConfirm" :
                    alert("비밀번호가 확인되지 않습니다."); break;
                case "memberNickname" :
