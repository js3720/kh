...
if(imageInput !=null){  // 화면에 imageInput이 있을 경우
    ...
    // 프로필 이미지가 출력되는 img태그에 src속성을 저장
    ...
    imageInput.addEventListener("change", e=>{
        ...
        console.log(e.target); // input (이벤트 발생한 객체)
        console.log(e.target.value); // 업로드된 파일 경로 (이벤트 발생 객체의 값)
        console.log(e.target.files); // 업로드된 파일의 정보가 담긴 배열 (이벤트 발생 객체가 가지고있는 파일을 배열로 가져옴)
