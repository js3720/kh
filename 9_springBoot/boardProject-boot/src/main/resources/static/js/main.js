...
function getCookie(key){
    ...
    // saveId=user@kh.or.kr; test=가나다; aaa=100;
    // 배열.map() : 배열의 모든 인덱스에 순차 접근하여 함수 수행 후
    //              수행 결과를 이용해서 새로운 배열을 만드는 함수
    const cookiesList = cookies.split("; ").map(cookie => cookie.split("="));
    
    const obj = {}; // 비어있는 객체 생성

    for(let i=0; i<cookiesList.length; i++){
        obj[cookiesList[i][0]] = cookiesList[i][1];
    }
    return obj[key];
}
