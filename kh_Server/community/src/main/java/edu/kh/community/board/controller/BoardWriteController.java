...
public class BoardWriteController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			// Enumeration 형태로 변환(Iterator의 과거 버전)
			// --> 해당 객체에 여러 값이 담겨 있고 이를 순서대로 얻어오는 방법을 제공
			//	   (보통 순서가 없는 모음(Set과 같은 경우)에서 하나씩 거낼 때 사용)
			...
			while(files.hasMoreElements()) { // 다음 요소가 있으면 true
				...
				// + 업오드가 안된 file 타입 태그의 name도 얻어와짐
				...
				if(rename!=null) { // 업로드된 파일이 있을 경우 == 
									...
									// 원본 또는 변경된 파일명을 얻어왔을 때 그 값이 null이 아닌경우
					// 이미지 정보를 담은 객체(BoardImage)를 생성
					BoardImage image = new BoardImage();
					image.setImageOriginal(original); // 원본명 (다운로드 시 사용)
					image.setImageReName(folderPath + rename); // 폴더경로 + 변경명
					image.setImageLevel(Integer.parseInt(name)); // 이미지 위치(0은 썹네일)
			...
			int boardCode = Integer.parseInt(mpReq.getParameter("type")); // hidden에서 가져옴
			...
			// --- 게시글 작서에 필요한 기본 파라미터 얻어오기 끝 ---------------------
			
			BoardService service = new BoardService();
			
			// 모두(insert/update)에 따라서 추가 파라미터 얻어오기 및 서비스 호출
