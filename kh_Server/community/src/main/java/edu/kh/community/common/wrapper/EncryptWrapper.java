...
public class EncryptWrapper extends HttpServletRequestWrapper{
   ...
   // -> Wrapper 클래스 생성시 반드시 HttpServletRequest 객체를 매개변수로 전달해야 한다.
   ...
   private String getSha512(String pw) {
      ...
      try {
         ...
         // 3. md를 이용해 암화를 진행할 수 있도록 Pw를 byte[]형태로 변환
         byte[] bytes = pw.getBytes(Charset.forName("UTF-8"));
         
         // 4. 암호화 수행 -> 암호와 결과가 md 내부에 저장됨
         md.update(bytes);
