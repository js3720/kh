/* VIEW
    ...
    *** VIEW 사용 시 주의사항 ***
    1) 가상의 테이블(실체X)이기 때문에 ALTER 구문 사용 불가능
    2) VIEW를 이용한 EML(INSERT, UPDATE, DELETE)가 가능한 경우도 있지만
    ...
    -- 1) OR REPLACE 옵션 : 기존에 동일한 뷰 이름이 존재하는 경우 덮어쓰고,
                            존재하지 않으면 새로 생성
    -- 2) FORCE / NOFORCE 옵션
    --    RORCE : 서브쿼리에 사용된 테이블이 존재하지 않아도 뷰 생성
...
-- 권한 생성 후 View V_EMP이(가) 생성되었습니다.

--> 각자 이니셜 계정에 view 생성 권한을 부여 ( 반드시 관리자 계정으로 실행 )
GRANT CREATE VIEW TO kjs; -- Grant을(를) 성공했습니다.
...
-- 삽입 확인
SELECT * FROM V_DCOPY2; --> 뷰에 삽입된걸 확인함

SELECT * FROM DEPT_COPY2;
--> VIEW에 삽입한 내용이 원본 테이블에 존재함
--> VIEW를 이용한 DML 구문이 원본에 영향을 미친다.

-- VIEW를 이용한 DML사용시 발생하는 문제점 == 제약조건 위배 현상
ROLLBACK;
SELECT * FROM DEPT_COPY2;
SELECT * FROM V_DCOPY2;
...
-- ORA-01400: NULL을 ("KJS"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
 ...
 *** SEQUENCE를 사용하는 방법 : PRIMARY KEY 컬럼에 사용될 값을 생성하는 용도로 사용 ***
  ...
  -- 시퀀스의 캐시 메모리는 할당된 크리만큼 미리 다음 값들을 생성해 저장해둠
  --> 시퀀스 호출 시 미리 저장되어진 값들을 가져와 반환하므로
      매번 시퀀스를 생성해서 반환하는 것보다 DB 속도가 향상 됨
...
-- EMPLOYEE_COPY4의 EMP{ID 컬럼에 PK 제약조건 추가
ALTER TABLE EMPLOYEE_COPY4
ADD PRIMARY KEY (EMP_ID);

SELECT * FROM EMPLOYEE_COPY4;

-- 223 부터 5씩 증가하는 스퀀스 생성
...
-- NEXTVAL 호출 1
SELECT SEQ_EMP_ID.NEXTVAL FROM DUAL; -- 최초 호출 == START WITH 조회
-- 223
...
--SEQ_EMP_ID의 증가값을 5 0> 1로 변경
ALTER SEQUENCE SEQ_EMP_ID
INCREMENT BY 1; -- Sequence SEQ_EMP_ID이(가) 변경되었습니다.

SELECT SEQ_EMP_ID.NEXTVAL FROM DUAL;

-- SEQUENCE 삭제
DROP SEQUENCE SEQ_EMP_ID; -- Sequence SEQ_EMP_ID이(가) 삭제되었습니다.
...
- 조회 시 전체 테이블 내용을 조회하는 것이 아닌
  인덱스가 지정된 컬럼만은 이용해서 조회하기 때문에
  시스템 부하가 낮아져 전체적인 성능에 향상된다.
  
* 인덱스 단점
- 데이터 변경(UPDATE, INSERT, DELETE) 작업이 빈번한 경우 오히려 성능이 저하되는 문제가 발생
- 인덱스도 하나의 객체이다보니 이를 저장하기 위한 별도 
