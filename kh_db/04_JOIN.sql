...
/* 관계형 데이터베이스에서 SQL을 이용해 테이블간 '관계'를 맺는 방법
 ...
 원하는 정보를 테이블에서 조회하려면 한개 이상의 테이블에서
 데이터를 읽어와야 되는 경우가 많다.
 이 때, 테이블간 관계를 맺기 위한 연결고리 역할이 필요한데,
 두 테이블에서 같은 데이터를 저장하는 컬럼의 연결고리가 됨
*/
...
-- 직원번호, 직 원명, 부서코드는 EMPLOYEE 테이블에 조회 가능
...
-- 여기서 재정한 표준 중 가장 유명한 것이 ASCII코드이다.
...
-- EMPLOYEE 테이블에 DEPT_CODE컬럼과 DEPARTMENT 테이블에 DEPT_ID컬럼은
-- 서로 같은 부서 코드를 나타낸다.
...
WHERE DEPT_CODE = DEPT_ID; -- WHERE 절에 같은 값을 가지고 있는 컬럼을 작성 (EQUAL JOIN이라고도 한다)
...
-- 3. 교차 조인(CROSS JOIN == CARTESIAN PRODUCT) -- 잘 안쓰긴 하지만 알아두자!
...
-- 4. 비등가 조인(NON EQAUL JOIN) -- 잘 안쓰긴 하지만 알아두자!
-- '='(등호)를 사용하지 않는 조인문
