...
--            ALTER TABLE 테이블명 MODIFY 컬럼명 SEFAULT '값'; (기본값 변경)
--> ** 데이터 타입 수정 시 컬럼에 저장된 데이터의 크기 미만으로는 변경할 수 없다.

-- 컬럼 삭제 : ALTER TABLE 테이블명 DROP (삭제할컬럼명);
--            ALTER TABLE 테이블명 DROP COLUMN 삭제할컬럼명;
-- 테이블이란? 행과 열로 이루어진 데이터베이스의 가장 기본적인 객체이기 때문에 
-- 마지막 남은 컬럼은 삭제 할 수 없다.
...
SELECT * FROM DEPT_COPY; --> LNAME 컬럼 추가 확인

-- (수정)
-- DEPT_COPY 테이블의 DEPT_ID 컬럼의 데이터 타입을  CHAR(2) -> VARCHAR2(3)
...
SELECT * FROM DEPT_COPY; -- 컬럼 1개 남는 것 확인

ALTER TABLE DEPT_COPY DROP(DEPT_ID);
-- ORA-12983: 테이블에 모든 열들을 삭제할 수 없습니다

ROLLBACK; -- 트랜잭션을 삭제하고 마지막 커밋 상태로 돌아감

SELECT * FROM DEPT_COPY; --> ROLLBACK 안됨..!
-- 왜? ************** CREAT / ALTER / DROP 같은 DDL은 ROLLBACK의 대상이 아님!!! **************
...
SELECT * FROM DEPT_COPY; -- 삽입 확인
ROLLBACK; -- 되돌리기
SELECT * FROM DEPT_COPY; -- 되돌린 것 확인
...
ROLLBACK; -- 되돌리기
SELECT * FROM DEPT_COPY; -- DDL 사용시 선행 DML 내용을 DB에 자동 COMMIT 되었기 때문에 되돌아가지 않음
...
--> 제약 조건까지 모두 삭제
--  원래는 FK 제약조건으로 인해 삭제가 불가능 하지만, 제약조건을 업애버려서 FK관계를 해제
...
ALTER TABLE DEPT_COPY RENAME CONSTRAINT PK_DCOPY TO DEPT_COPY_FK;

-- 3) 테이블명 변경 : ALTER TABLE 테이블명 RENAME TO 변경면;
ALTER TABLE DEPT_COPY RENAME TO DCOPY;
SELECT * FROM DCOPY;
SELECT * FROM DEPT_COPY; -- 변경되어 없는 테이블이라서 조회 불가
