...
SELECT DEPT_CODE, SUM(SALARY) FROM EMPLOYEE GROUP BY DEPT_CODE; -- 부서별 합계 조회

SELECT DEPT_CODE, SUM(SALARY), ROUND(AVG(SALARY)),COUNT(*) FROM EMPLOYEE GROUP BY DEPT_CODE ORDER BY DEPT_CODE;

SELECT DEPT_CODE, COUNT(BONUS) FROM EMPLOYEE GROUP BY DEPT_CODE ORDER BY DEPT_CODE;

-- EMPLOYEE 테이블에서
-- 성별과 성별 별 급여 평균(정수처리), 급여합계, 인원 수 조회하고
-- 인원수로 내림차순 정렬

SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남자','2','여자') "성별",
FLOOR(AVG(SALARY)) "급여 평균",
SUM(SALARY) "급여 합계", COUNT(*)"인원 수" FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1),'1','남자','2','여자')
ORDER BY "인원 수" DESC;

-- * WHERE절 GROUP BY 절을 혼합하여 사용
--> WHERE절은 각 컬럼 값에 대한 조건 ( SELECT문 해석 순서를 잘 기억하는 것이 중요!!)

-- EMPLOYEE 테이블에서 부서코드가 'D5'랑 'D6'인 부서의 평균 급여 조회
...
FROM EMPLOYEE
WHERE TO_NUMBER(SUBSTR(EMP_NO,1,2),'99') BETWEEN 70 AND 79
GROUP BY DEPT_CODE
...
-- EMPLOYEE 테이블에서 각 부서에 소속된 직급 별 급여합,
-- 부서 별 급여합, 전체 직원 급여 총합 조회
...
-- 초보자들이 사용하기 쉽다. 
-- (주의) 집합 연산에 사용되는 SELECT문은 SELECT절이 동일 해야함

-- UNION은 OR 같은 개념 (합집합) --> 중복 제거
-- INTERSECT는 AND 같은 개념 (교집합)
-- UNION ALL은 OR 결과 값에 AND 결과값이 더해진거(합집합+ 교집합)--> 중복미제거
-- MINUS는 차집합 개념
...
-- MINUS : 선행 SELECT 1 결과에서 다음 SELECT 2 결과와 겹치는 부분을 제외한
--         나머지 부분만 추출(차집합)
-- 부서코드가 D5중 급여가 300만 초과인 직원 제외
/*1*/SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'

MINUS -- 차집합

/*2*/SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > 3000000;

-- MINUS는 AND연산 + 비교 연산 반대로 작성하면 된다
SELECT EMP_ID,EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
WHERE DEPT_CODE ='D5'
AND SALARY <=3000000;
