...
CREATE TABLE USER_USED_UK2(
    ...
    -- 테이블 레벨 제약조건 설정 부분
    -- UNIQUE(USER_ID) -- 테이블 레벨 제약조건 설정(제약조건명 미지정)
    CONSTRAINT USER_ID_NAME_U UNIQUE(USER_ID, USER_NAME) -- 테이블 레벨 제약조건 설정(제약조건명 지정)
);
...
-- 컬럼이 모두 중복되는 값일 경우에만 오류 발생
-- ORA-00001: 무결성 제약 조건(KJS.USER_ID_NAME_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KJS.PK_USERNO_USERID)에 위배됩니다
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER, --CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼레벨 FK설정
  CONSTRAINT GRADE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE -- 테이블레벨 FK설정
);
...
-- ORA-02291: 무결성 제약조건(KJS.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
             --> 참조 무결성 위배
...
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(KJS.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- 2) ON DELETE SET NULL : 부모키 삭제시 자식키를 NULL로 변경하는 옶션
...
-- '남자'라는 조건 이외의 값이 들어와서 에러 발생
-- ORA-02290: 체크 제약조건(KJS.GENDER_CHECK)이 위배되었습니다
...
-- (컬럼명, 데이터 타입, 값)이 복사되고 제약조건은 NOT NULL만 복사됨
