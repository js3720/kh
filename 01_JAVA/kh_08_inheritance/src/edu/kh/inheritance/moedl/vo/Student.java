...
public class Student extends Person{ // 클래스의 추상화 (중복되는 불필요한 코드를 최소화)
    ...
    public Student(){
        ...
        // *super() 생성자 때문에
        // 반드시 자식 내부에 부모 객체가 생성된다!

        // *super() 생성자 미작성 시
        // 컴파일러가 컴파일 단계에서 자동으로 추가해줌
    }
    public Student(String name, int age, String nationality, int grade, int classRoom){
        // 전달받은 값 중 name, age, nationality 부모 필드에 세팅하기
        // this.name = name; // (X)
        // 상속 받은 부모의 필드(name)를
