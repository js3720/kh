...
public class ImageDeleteScheduling {
	...
	// 일(5초) - 10초대기 - 일(5초) - 10초대기
	
	// @Scheduled(fixedRate = 10000) // ms단위
	// 일(5초)
	// 대기(10초)
	
	// cron = "초 분 시 일 월 요일 [년도]"
	//@Scheduled(cron="0,30 * * * * * ")
	@Scheduled(cron="0 0 * * * * ") // 매 시간 정각마다 (*시 0분 0초)
	public void test() {
		...
		if(!serverImageList.isEmpty()) {
			...
			for(File server : serverImageList) {
				...
				// 6) 서버에 존재하는 파일이
				// 	  DB에(dbImageList)에 없다면 삭제
				//String[] temp = server.toString().split("\\\\");
				//String s = temp[temp.length-1];
				
				// System.out.println(server.getName());
				
				// ListindexOf(객체) = 객체가 List에 있으면 해당 인덱스 반환, 없으면 -1 반환
