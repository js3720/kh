...
public class MainService {
    ...
    public void signUp() {
        ...
        for(Member m : members){
            if(m.getMemberName().equals(id)) flag = false;
        }
        if(flag) {
    ...
    public void login() {
        ...
        System.out.print("비밀번호 :");
        String pw = sc.next();
        for(Member m : members){ // 누군가가 회원가입을 했는데,
            if(m.getMemberId().equals(id) && m.getMemberPw().equals(pw)){
                loginMember = m; // 현재 로그인 했는지 판별하는 로그인맴버 참조변수에 현재 객체의 주소값을 얕은복사
                flag = false;
            }
        }
